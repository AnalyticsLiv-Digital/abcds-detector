...
"""Module with long-form ABCD feature configurations adapted for Shorts scoring."""
...
VIDEO_SHORTS_SCORE_EXCLUDED_FEATURE_IDS = {
...
def is_video_shorts_score_excluded_feature(feature_id: str) -> bool:
  """Returns True when a feature should be analysed but not scored."""
  return feature_id in VIDEO_SHORTS_SCORE_EXCLUDED_FEATURE_IDS


def get_video_shorts_feature_configs() -> list[VideoFeature]:
  """Gets long-form ABCD prompts for Shorts video evaluation.

  The listed excluded features are still analysed by the LLM, but they are
  marked with include_in_evaluation=False so they do not contribute to the
  final Shorts score.
  ...
  for feature_config in feature_configs:
    if is_video_shorts_score_excluded_feature(feature_config.id):
      feature_config.include_in_evaluation = False
